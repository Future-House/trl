...
class GRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.args.use_vllm:
            ...
            local_prompts_text = gather_object(prompts_text)
            if self.accelerator.is_last_process:
                outputs = self.llm.generate(local_prompts_text, sampling_params=self.sampling_params, use_tqdm=False)
                completion_ids = [out.token_ids for completions in outputs for out in completions.outputs]
            else:
                completion_ids = [None] * (len(local_prompts_text) * self.num_generations)

            # Broadcast the completions from the main process to all processes, ensuring each process receives its
            # corresponding slice.
            completion_ids = broadcast_object_list(completion_ids, from_process=self.accelerator.num_processes - 1)
            process_slice = slice(
